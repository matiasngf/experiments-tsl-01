...
const ALPHA_SQ = 0.5;
const DAMPING = 0.996; // Energy loss per frame (1.0 = no damping)
  ...
  const drawMaterial = useMaterial(
    ...
    (mat) => {
      const colorFn = Fn(() => {
        ...
        const textureVariation = surfaceNoise.mul(noiseAmount); // 0.6 to 1.4
        const mouseInfluence = shapedInfluence.mul(textureVariation);

        // Add displacement when mouse is moving
        ...
        const mouseDisplacement = isMoving.select(
          mouseInfluence.mul(mouseStrength),
          float(0)
        );
  ...
  const screenMaterial = useMaterial(
    ...
    (mat) => {
      const colorFn = Fn(() => {
        ...
        return vec4(vec3(positive, negative, noiseVis), float(1));
      });
